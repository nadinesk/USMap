{-# LANGUAGE NoMonomorphismRestriction, FlexibleContexts,  GADTs, TypeFamilies  #-}
...
main :: IO ()
-- main takes in csv file, 
-- names two variables: replacedPath: file name with new file name convention; just Title: for title on top of SVG to be the file name, by dropping csv extension, 
-- runs makeSVGwTitle: xs is: on the right side of =<<, parses csv from replacedPath given in getArgs, drops the first row (title) in drop 1, eliminates the either from the parsed csv, and shows error if it's not done; title is: gets just the title name without the extension on the csv file, then renders the SVG from doIt 1 
main = do 
   csvFilePath:_ <- getArgs
   let replacedPath = replaceColon csvFilePath  
       justTitle = dropExtension $ takeFileName replacedPath
   makeSVGwTitle stateStuff justTitle   . eliminateTheEither . fmap (drop 1) =<< parseCSVFromFile replacedPath


--calls d function on makeTheDiagram, which renders svg 
--makeSVGwTitle :: [StateStuff] -> String -> [[String]] -> IO ()    
makeSVGwTitle stateInfos title xs = makeSVG $ makeTheDiagram xs stateInfos title 

makeSVG :: Diagram SVG R2 -> IO () 
makeSVG x = do
    --first: taking in csv file name from console
    --second: redo file name 
    --third: make svg file with replaced csv file name 
    csvFilePath:_ <- getArgs 
    let replacedPath = replaceColon csvFilePath
    renderSVG (replaceExtension replacedPath "svg")(Dims 800 800) x

-- eliminates either
eliminateTheEither :: Show a => Either a t -> t
...
--turns list of list of states and values as tuples (made with toTuple) to list of states and values tuples
--toPairs :: [[String]] -> [State]
toPairs xs = mapMaybe toTuple xs

--takes list and turns into tuples, i.e., from ["blah", "2"] to maybe ("blah", "2"): is that right???
toTuple :: [a] -> Maybe (a,a)
toTuple xs = case xs of
        x:y:_ -> Just (x,y)
        _ -> Nothing

--multiplies 0.13 by the values associated with each state
colorVal :: (Read a, Fractional a, Eq a1) => a1 -> [(a1, String)] -> a
colorVal name pairs = 0.013* (read (fromJust $ lookup name pairs))


color :: (Floating a, Ord a) => a -> Colour a
color x = blend x red yellow

-- diagramState :: Renderable (Path R2) b => Trail' Loop R2 -> R2 -> Colour Double ->  Diagram b R2
diagramState stateCoords globalPos fillColor 
    = stateCoords #scaleY (-1) # strokeLoop #lc white #fc fillColor #lwG 0.5 #translate globalPos


--akfigure :: Renderable (Path R2) b => Colour Double -> Diagram b R2
--akfigure x = strokedLine #scaleY (-1) #fc x #lc white  #lwG 0.5 #translate (158.0767 ^& (-453.67502))
-- kyfigure x = kyStrokedLine # scaleY (-1) #fc x #lc white #lwG 0.5  #translate  (725.9944 ^& (-295.2707))

-- diagramBezier stLn globalPos fillColor = 
    
    

pointstoVector (a, b) = a ^& b 

stateStuff ::  [StateStuff]
stateStuff = [ (gluedLine, (158.0767, (-453.67502)), "Alaska")
             , (al       , (631.30646, (-460.4157)), "Alabama")
             , (az       , (144.9112, (-382.6291)), "Arizona")
             , (ar       , (593.82477, (-343.05295)), "Arkansas")
             , (ca       , (144.69443, (-382.19812)), "California")
             , (ca1      , (56.55921400000001, (-338.48145)), "California")
             , (ca2      , (58.49914600000001, (-338.48145)), "California" )
             , (ca3      , (79.19176000000002, (-358.28493)), "California")
             , (co       , (380.03242, (-320.96457)), "Colorado")
             , (ct       , (874.0683, (-178.86288)), "Connecticut")
             , (de       , (825.6261, (-228.2791)), "Delaware")
             , (dc       , (805.81946, (-250.84384)), "District of Columbia")
             , (fl1      , (759.8167, (-439.1428)), "Florida")
             , (fl2      , (772.3621099999999, (-571.5478800000001)), "Florida")
             , (fl3      , (785.8608099999999, (-566.5363900000001)), "Florida")
             , (ga       , (672.2923, (-355.5518)), "Georgia")
             , (hi1      , (233.08751, (-519.3095)), "Hawaii")
             , (hi2      , (243.27217000000002, (-515.59129)), "Hawaii")
             , (hi3      , (273.9878,(-525.61429)), "Hawaii")
             , (hi4      , (294.19545, (-534.50566)), "Hawaii")
             , (hi5      , (299.53027, (-539.67881)), "Hawaii")
             , (hi6      , (306.4817,(-538.54718)), "Hawaii")
             , (hi7      , (323.13281, (-554.06665)), "Hawaii")
             , (il       , (619.54145, (-300.34244)), "Illinois")
             , (ind      , (619.5695,(-299.9713)), "Indiana")
             , (ia       , (569.1915,(-199.5843)), "Iowa")
             , (ks       , (507.88058, (-324.38028)), "Kansas")
             , (ma       , (899.6235,(-173.25394)), "Massachusetts" )
             , (ma1      , (890.2499499999999, (-174.05412)), "Massachusetts")
             , (ma2      , (855.4508199999999, (-152.06593)), "Massachusetts")
             , (sp       , (581.6193, (-82.059006)), "Michigan")
             , (sp1      , (567.49209, (-111.21318)), "Michigan")
             , (mn       , (475.23782, (-128.82439)), "Minnesota")
             , (ms       , (631.55884, (-459.34457)), "Mississippi")
             , (mo       , (558.44025, (-248.11316)), "Missouri")
             , (mt       , (369.20952, (-56.969133 )), "Montana")
             , (ne       , (486.09787, (-240.70058)), "Nebraska")
             , (nv       , (196.39273, (-185.57552)), "Nevada")
             , (nh1      , (880.799, (-142.42476)), "New Hampshire")
             , (nj       , (829.67944, (-188.46016)), "New Jersey")
             , (nm       , (288.15256, (-424.01315)), "New Mexico")
             , (ny       , (830.37946, (-188.7456)), "New York" )
             , (nc       , (839.25199, (-327.52211)), "North Carolina" )
             , (nd       , (475.30527, (-128.91846)), "North Dakota")
             , (ok       , (380.34314, (-320.82147)), "Oklahoma")
             , (oregon   , (148.72185, (-175.53152)), "Oregon")
             , (pa       , (825.1237, (-224.69205)), "Pennsylvania")
             , (ri       , (874.07, (-178.89536)), "Rhode Island")
             , (sc       , (764.9433, (-408.1649)), "South Carolina")
             , (sd       , (476.44687, (-204.02465)), "South Dakota")
             , (tx       , (361.46423, (-330.57358)), "Texas")
             , (tx1      , (460.49357999999995, (-560.23631)), "Texas")
             , (ut       , (259.49835, (-310.1051)), "Utah")
             , (vt       , (844.48413, (-154.0579)), "Vermont")
             , (va       , (831.63885, (-266.0689)), "Virginia")
             , (wa       , (102.07324, (-7.6117735)), "Washington")
             , (wi       , (615.0659, (-197.36867)), "Wisconsin")
             , (wy       , (360.37668, (-143.27586)), "Wyoming")
             ]
             

type StateStuff = (Trail' Loop R2, (Double, Double), [Char])
type StateCoords = Trail' Loop R2
type GlobalPos = (Double, Double)
type StateNamae = [Char]

type Row = [String]
type CSVer = [Row]

-- generalStDiagram :: (Renderable Diagrams.TwoD.Text.Text b, Renderable (Path R2) b) 
--                 => CSVer -> StateStuff -> Diagram b R2
generalStDiagram xs (stCoords, glPos, stName)  
        = diagramState stCoords (pointstoVector glPos) $ color $ colorVal stName $ toPairs xs

makeFewDiagram csv stateInfos = map (generalStDiagram csv) stateInfos 


-- makeTheDiagram :: (Renderable Diagrams.TwoD.Text.Text b, Renderable (Path R2) b) => String -> [[String]] -> Diagram b R2
makeTheDiagram xs stateInfos title   = mconcat $ 
            makeFewDiagram xs stateInfos
            ++ 
            [ --akfigure         $ color $ colorVal "Alaska"               $ toPairs xs
              idfigure         $ color $ colorVal  "Idaho"               $ toPairs xs
            , kyfigure         $ color $ colorVal  "Kentucky"            $ toPairs xs
            , lafigure         $ color $ colorVal  "Louisiana"           $ toPairs xs
            , mefigure         $ color $ colorVal  "Maine"               $ toPairs xs
            , mefigure1        $ color $ colorVal  "Maine"               $ toPairs xs
            , mefigure2        $ color $ colorVal  "Maine"               $ toPairs xs
            , mdfigure         $ color $ colorVal  "Maryland"            $ toPairs xs
            , mdfigure1        $ color $ colorVal  "Maryland"            $ toPairs xs
            , tnfigure         $ color $ colorVal  "Tennessee"           $ toPairs xs         
            , wvfigure         $ color $ colorVal   "West Virginia"      $ toPairs xs
            , ncfigure         $ color $ colorVal   "North Carolina"     $ toPairs xs       
            , vafigure1        $ color $ colorVal   "Virginia"           $ toPairs xs                                  
            , ohfigure         $ color $ colorVal   "Ohio"               $ toPairs xs                                  
            , mifigure         $ color $ colorVal   "Michigan"           $ toPairs xs                                  
            , mifigure2        $ color $ colorVal   "Michigan"           $ toPairs xs                                  
            , headerstuff title 
            ]   
...
-- example x = ejpts #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate  (380.03242 ^& (-320.96457))

--al :: (TrailLike c, V c ~ R2) => c
-- al :: Trail' Loop R2
...
cafigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
cafigure x = ca #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (144.69443 ^& (-382.19812))

ca1 :: (TrailLike c, V c ~ R2) => c
...
ca1figure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
ca1figure x = ca1 #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (56.55921400000001 ^& (-338.48145))

ca2 :: (TrailLike c, V c ~ R2) => c
...
ca2figure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
ca2figure x = ca2 #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (58.49914600000001 ^& (-338.48145))

ca3 :: (TrailLike c, V c ~ R2) => c
...
ca3figure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
ca3figure x = ca3 #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (79.19176000000002 ^& (-358.28493))
...
ca4figure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
ca4figure x = caStrokedLine # scaleY (-1) #fc x #lc white #lwG 0.5  #translate (77.73680500000002 ^& (-366.93379))

co :: (TrailLike c, V c ~ R2) => c
co = fromOffsets . map r2 $[(4.90324,-86.32496), (-113.38856,-12.64396), (-12.21382,87.93916), (120.69914,11.02976)]

cofigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
cofigure x = co #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate  (380.03242 ^& (-320.96457))

ct :: (TrailLike c, V c ~ R2) => c
...
ctfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
ctfigure x = ct #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (874.0683 ^&  (-178.86288))

de :: (TrailLike c, V c ~ R2) => c
...
defigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
defigure x = de #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (825.6261 ^& (-228.2791))

dc :: (TrailLike c, V c ~ R2) => c
dc = fromOffsets . map r2 $[ (-1.85828,-1.82417), (-1.23263,-0.68629), (1.44301,-2.02247), (2.88909,1.9485), (-1.24119,2.58443)]

dcfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
dcfigure x = dc #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (805.81946 ^& (-250.84384))

fl1 :: (TrailLike c, V c ~ R2) => c
...
flfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
flfigure x = mconcat 
         [ fl1 #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.01 #translate (759.8167 ^& (-439.1428))
         , fl2 #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.01 #translate (772.3621099999999  ^& (-571.5478800000001))
         , fl3 #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.01 #translate (785.8608099999999 ^& (-566.5363900000001))
        ]
ga :: (TrailLike c, V c ~ R2) => c
...
gafigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
gafigure x = ga #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (672.2923 ^& (-355.5518))

hi1 :: (TrailLike c, V c ~ R2) => c
hi1 = fromOffsets . map r2 $ [(1.93993,-3.55655), (2.26326,-0.32332), (0.32332,0.8083), (-2.1016,3.07157), (-2.42491,0.0)]

hi2 :: (TrailLike c, V c ~ R2) => c
...
hifigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
hifigure x = mconcat
         [ hi1 #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (233.08751 ^& (-519.3095))
         , hi2 #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (243.27217000000002 ^& (-515.59129))
         , hi3 #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (273.9878 ^& (-525.61429))  
        , hi4 #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (294.19545 ^& (-534.50566))
        , hi5 #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (299.53027 ^& (-539.67881))
        , hi6 #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (306.4817 ^& (-538.54718))
        , hi7 #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (323.13281 ^& (-554.06665))
        ]
...
idfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
idfigure x = idStrokedLine # scaleY (-1) #fc x #lc white #lwG 0.5  #translate (148.4788 ^& (-176.48395))

il :: (TrailLike c, V c ~ R2) => c
...
ilfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
ilfigure x = il #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (619.54145 ^& (-300.34244))

ind :: (TrailLike c, V c ~ R2) => c
...
indfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
indfigure x = ind #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (619.5695 ^& (-299.9713))

ia :: (TrailLike c, V c ~ R2) => c
...
iafigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
iafigure x = ia #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate  (569.1915 ^& (-199.5843))

ks :: (TrailLike c, V c ~ R2) => c
...
ksfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
ksfigure x = ks #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (507.88058 ^& (-324.38028))
...
lafigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
lafigure x = laStrokedLine # scaleY (-1) #fc x #lc white #lwG 0.5  #translate (607.96704 ^& (-459.16125))
...
mefigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
mefigure x = mestrokedLine #scaleY (-1) #fc x #lc white  #lwG 0.5 #translate (922.8398 ^& (-78.83072))

me :: (TrailLike c, V c ~ R2) => c
me = fromOffsets . map r2 $ [ (1.53578,-1.53578), (1.37412,1.0508), (0.56581,2.42492), (-1.69744,0.88913), (-1.77827,-2.82907)]

mefigure1 :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
mefigure1 x = me #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (901.90801 ^& (-108.978251))
...
mefigure2 :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
mefigure2 x = me2strokedLine #scaleY (-1) #fc x #lc white  #lwG 0.5 #translate (908.61694 ^& (-103.077631))
...
mdfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
mdfigure x = mdStrokedLine # scaleY (-1) #fc x #lc white #lwG 0.5  #translate (839.79175 ^& (-252.41476))
...
mdfigure1 :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
mdfigure1 x = md2StrokedLine # scaleY (-1) #fc x #lc black #lwG 0.5  #translate (823.82217 ^& (-261.44348))

ma :: (TrailLike c, V c ~ R2) => c
...
mafigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
mafigure x = ma #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (899.6235 ^& (-173.25394)) 


ma1 :: (TrailLike c, V c ~ R2) => c
ma1 = fromOffsets . map r2 $[ (2.28622,-2.62917), (1.60037,0.0), (1.82899,1.48605), (-2.40054,1.0288), (-2.17192,1.0288), (-1.14312,-0.91448)]

mafigure1 :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
mafigure1 x = ma1 #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (890.2499499999999 ^& (-174.05412))

ma2 :: (TrailLike c, V c ~ R2) => c
...
mafigure2 :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
mafigure2 x = ma2 #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (855.4508199999999 ^& (-152.06593))
...
mifigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
mifigure x = mistrokedLine #scaleY (-1) #fc x #lc white  #lwG 0.5 #translate (697.86005 ^& (-177.2369))

sp :: (TrailLike c, V c ~ R2) => c
...
mifigure1 :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
mifigure1 x = sp #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.01 #translate (581.6193 ^& (-82.059006))
...
mifigure2 :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
mifigure2 x = spstrokedLine #scaleY (-1) #fc x #lc white  #lwG 0.5 #translate (667.7936900000001 ^& (-114.18718999999999))

sp1 :: (TrailLike c, V c ~ R2) => c
...
mifigure3 :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
mifigure3 x = sp1 #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 1 #translate (567.49209 ^& (-111.21318))

mn :: (TrailLike c, V c ~ R2) => c
...
mnfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
mnfigure x = mn #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate  (475.23782 ^& (-128.82439))

ms :: (TrailLike c, V c ~ R2) => c
...
msfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
msfigure x = ms #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate  (631.55884 ^& (-459.34457))

mo :: (TrailLike c, V c ~ R2) => c
...
mofigure :: Renderable (Path R2) b =>
            Colour Double -> Diagram b R2
mofigure x = mo #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (558.44025 ^& (-248.11316))

mt :: (TrailLike c, V c ~ R2) => c
...
mtfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
mtfigure x = mt #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate  (369.20952 ^& (-56.969133 ))

ne :: (TrailLike c, V c ~ R2) => c
...
nefigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
nefigure x = ne #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (486.09787 ^& (-240.70058))

nv :: (TrailLike c, V c ~ R2) => c
...
nvfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
nvfigure x = nv #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate  (196.39273 ^& (-185.57552))

nh1 :: (TrailLike c, V c ~ R2) => c
...
nhfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
nhfigure x = nh1 #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (880.799 ^& (-142.42476))

nj :: (TrailLike c, V c ~ R2) => c
...
njfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
njfigure x = nj #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (829.67944 ^& (-188.46016))

nm :: (TrailLike c, V c ~ R2) => c
...
nmfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
nmfigure x = nm #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (288.15256 ^& (-424.01315))

ny :: (TrailLike c, V c ~ R2) => c
...
nyfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
nyfigure x = ny #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (830.37946 ^& (-188.7456)) 
...
ncfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
ncfigure x = ncStrokedLine # scaleY (-1) #fc x #lc white #lwG 0.5  #translate  (834.9815 ^& (-294.31555))

nc :: (TrailLike c, V c ~ R2) => c
...
ndfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
ndfigure x = nd #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (475.30527 ^& (-128.91846))
...
ohfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
ohfigure x = ohStrokedLine # scaleY (-1) #fc x #lc white #lwG 0.5  #translate  (735.32495 ^& (-193.32832)) 

ok :: (TrailLike c, V c ~ R2) => c
...
okfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
okfigure x = ok #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (380.34314 ^& (-320.82147))

oregon :: (TrailLike c, V c ~ R2) => c
...
oregonfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
oregonfigure x = oregon #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (148.72185 ^& (-175.53152))

pa :: (TrailLike c, V c ~ R2) => c
...
penfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
penfigure x = pa #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (825.1237 ^& (-224.69205))

ri :: (TrailLike c, V c ~ R2) => c
...
rifigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
rifigure x = ri #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (874.07 ^& (-178.89536))

sc :: (TrailLike c, V c ~ R2) => c
...
scfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
scfigure x = sc #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (764.9433 ^& (-408.1649))

sd :: (TrailLike c, V c ~ R2) => c
...
sdfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
sdfigure x = sd #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (476.44687 ^& (-204.02465))
...
tnfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2

tnfigure x = tnStrokedLine # scaleY (-1) #fc x #lc white #lwG 0.5  #translate  (696.67786 ^& (-318.25412))

tx :: (TrailLike c, V c ~ R2) => c
...
txfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
txfigure x = tx #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (361.46423 ^& (-330.57358))

tx1 :: (TrailLike c, V c ~ R2) => c
...
txfigure1 :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
txfigure1 x = tx1 #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (460.49357999999995 ^& (-560.23631))


ut :: (TrailLike c, V c ~ R2) => c
...
utfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
utfigure x = ut #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (259.49835 ^& (-310.1051))

vt :: (TrailLike c, V c ~ R2) => c
...
vtfigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
vtfigure x = vt #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (844.48413 ^& (-154.0579))

va :: (TrailLike c, V c ~ R2) => c
...
vafigure :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
vafigure x = va #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate  (831.63885 ^& (-266.0689)) 
...
vafigure1 :: Renderable (Path R2) b =>
             Colour Double -> Diagram b R2
vafigure1 x = vaStrokedLine # scaleY (-1) #fc x #lc white #lwG 0.5  #translate (834.97904 ^& (-294.37028))

wa :: (TrailLike c, V c ~ R2) => c
...
wafigure :: Renderable (Path R2) b =>
            Colour Double -> Diagram b R2
wafigure x = wa #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (102.07324 ^& (-7.6117735))
...
wafigure1 ::  Renderable (Path R2) b =>
              Colour Double -> Diagram b R2
wafigure1 x = waStrokedLine # scaleY (-1) #fc x #lc white #lwG 0.5  #translate (92.616548 ^& (-13.5907385))
...
wvfigure :: Renderable (Path R2) b =>
            Colour Double -> Diagram b R2
wvfigure x = wvStrokedLine # scaleY (-1) #fc x #lc white #lwG 0.5  #translate (761.1855 ^& (-238.96732))

wi :: (TrailLike c, V c ~ R2) => c
...
wifigure :: Renderable (Path R2) b =>
            Colour Double -> Diagram b R2
wifigure x = wi #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (615.0659 ^& (-197.36867))


wy :: (TrailLike c, V c ~ R2) => c
wy = fromOffsets . map r2 $[  (-106.7426,-13.45706), (-14.08348,88.45803), (113.26461,13.58549), (7.56147,-88.58646)]

wyfigure :: Renderable (Path R2) b =>
            Colour Double -> Diagram b R2
wyfigure x = wy #scaleY (-1)  # strokeLoop #lc white #fc x #lwG 0.5 #translate (360.37668 ^& (-143.27586))
...
s :: (HasStyle b, Transformable b, TrailLike b, V b ~ R2) => b
s = square 10 # lwL 0

textlow :: (Renderable Text b, Renderable (Path R2) b) => Diagram b R2
textlow = text "0%" # fontSize (Local 10) <> rect 30 5 # lwL 0 

textmid :: (Renderable Text b, Renderable (Path R2) b) => Diagram b R2
textmid = text "50%" # fontSize (Local 10) <> rect 550 5 # lwL 0

texthigh :: (Renderable Text b, Renderable (Path R2) b) => Diagram b R2
texthigh = text "100%" # fontSize (Local 10)  <> rect 30 5 # lwL 0
...
textTitle :: (Renderable Text b, Renderable (Path R2) b) => String -> Diagram b R2
textTitle x = text x # fontSize (Local 30) <> rect 250 50 # lwL 0
...
legendstuff :: (Monoid a, HasOrigin a, Juxtaposable a, HasStyle a, Transformable a, TrailLike a, Semigroup a, V a ~ R2) => a
legendstuff = hcat $ zipWith fc legendcolors (repeat s)

horizontalText :: (Renderable Text b, Renderable (Path R2) b) => Diagram b R2
horizontalText = hcat 
                    [
                     textlow 
                    , textmid
...
lgdstuffblah :: (Renderable Text b, Renderable (Path R2) b) => Diagram b R2
...
headerstuff :: (Renderable Text b, Renderable (Path R2) b) => String -> Diagram b R2
